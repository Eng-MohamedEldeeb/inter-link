...
export class User {
  ...
  private static readonly schemaFactory = () => {
    ...
    this.schema.virtual("totalFollowing").get(function () {
      if (this.following) return this.following.length
      return 0
    })

    this.schema.virtual("totalFollowers").get(function () {
      if (this.followers) return this.followers.length
      return 0
    })

    this.schema.virtual("birthDate").set(function (v) {
      return this.set(
        "age",
        new Date().getFullYear() - new Date(v).getFullYear(),
      )
    })
    ...
    this.schema.post("save", async function (res) {
    ...
    this.schema.pre("findOneAndUpdate", async function (next) {
    ...
    this.schema.post(
      "findOneAndDelete",
      async function ({ _id, avatar }: IUser) {
        Promise.allSettled([
