...
export class Group {
  private static readonly schema = new Schema<IGroup>(
    {
      ...
      description: {
        ...
        maxlength: [
          700,
          "Group's description can't be more than 700 characters",
        ],
        required: [true, "Group's name is required"],
      },
  ...
  private static readonly schemaFactory = () => {
    this.schema.virtual("totalMembers").get(function (this: IGroup) {
      if (this.members) return this.members.length
      return 0
    })

    this.schema.post("findOneAndDelete", async function (res: IGroup) {
      const { cover } = res
